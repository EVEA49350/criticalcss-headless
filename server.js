...
// Cache mémoire simple (utile pour éviter de recalculer en boucle)
...
function cacheSet(key, value) {
  ...
  if (CACHE.size >= CACHE_MAX_ITEMS) {
    // delete oldest-ish: Map preserves insertion order
...
// GET /critical-css?url=https://...&w=1366&h=768&token=...
app.get("/critical-css", async (req, res) => {
  ...
  try {
    browser = await chromium.launch({
      // Sur container, le sandbox est généralement OK avec l'image Playwright,
      // mais on évite d'ajouter des flags agressifs ici.
      headless: true
    });
    ...
    // Récupération du texte des stylesheets
    ...
    for (const s of sheets.headers || []) {
      try {
        ...
        // certains sheets peuvent être inaccessibles
      }
    }
